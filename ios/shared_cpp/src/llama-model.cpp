...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    if (arch == LLM_ARCH_LUMINA2) {
        ml.get_key(LLM_KV_CONTEXT_LENGTH, hparams.n_ctx_train, false);
        if (hparams.n_ctx_train == 0) {
            hparams.n_ctx_train = 1024;
        }
        ...
        ml.get_key(LLM_KV_BLOCK_COUNT, hparams.n_layer, false);
    ...
    // n_layer already handled above if Lumina2
    if (arch != LLM_ARCH_LUMINA2) {
        // redundant but safe since we changed the block above
    }
    ...
    if (arch == LLM_ARCH_LUMINA2 && hparams.n_ff_arr[0] == 0) {
        std::fill(hparams.n_ff_arr.begin(), hparams.n_ff_arr.begin() + hparams.n_layer, 6912);
    }

    ml.get_key_or_arr(LLM_KV_ATTENTION_HEAD_COUNT, hparams.n_head_arr, hparams.n_layer, false);
    if (arch == LLM_ARCH_LUMINA2 && hparams.n_head_arr[0] == 0) {
        std::fill(hparams.n_head_arr.begin(), hparams.n_head_arr.begin() + hparams.n_layer, 32);
    }
        ...
        case LLM_ARCH_LUMINA2:
            {
                ml.get_key(LLM_KV_CONTEXT_LENGTH, hparams.n_ctx_train, false);
                ml.get_key(LLM_KV_EMBEDDING_LENGTH, hparams.n_embd, false);
                ml.get_key(LLM_KV_FEED_FORWARD_LENGTH, hparams.n_ff_arr[0], false);
                ml.get_key(LLM_KV_ATTENTION_HEAD_COUNT, hparams.n_head_arr[0], false);
                ml.get_key(LLM_KV_ATTENTION_HEAD_COUNT_KV, hparams.n_head_kv_arr[0], false);

                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps, false);
                hparams.causal_attn = false;

                // lumina2 specific defaults if not in GGUF
                if (hparams.n_ctx_train == 0) hparams.n_ctx_train = 1024;
                if (hparams.n_embd == 0) hparams.n_embd = 2048;
                if (hparams.n_layer == 0) hparams.n_layer = 28;
                if (hparams.n_head_arr[0] == 0) {
                    std::fill(hparams.n_head_arr.begin(), hparams.n_head_arr.end(), 32);
                }
                if (hparams.n_head_kv_arr[0] == 0) {
                    std::fill(hparams.n_head_kv_arr.begin(), hparams.n_head_kv_arr.end(), 32);
                }
                if (hparams.n_ff_arr[0] == 0) {
                    std::fill(hparams.n_ff_arr.begin(), hparams.n_ff_arr.end(), 6912);
                }

                switch (hparams.n_layer) {
                    case 28: type = LLM_TYPE_2B; break;
                    default: type = LLM_TYPE_UNKNOWN;
                }
            } break;
        case LLM_ARCH_RND1:
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor_qkv = [&](llama_layer & layer, int bid,
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_LUMINA2:
            {
                llm = std::make_unique<llm_build_lumina2>(*this, params);
            } break;
        case LLM_ARCH_RND1:
            {
